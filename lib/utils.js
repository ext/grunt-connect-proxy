...
utils.matchContext = function(context, url) {
    ...
    const positiveContexts = _.filter(contexts, function(c) {
        return c.charAt(0) !== "!";
    });
    ...
    const negativeMatch = _.find(negativeContexts, function(c) {
    ...
    const positiveMatch = _.find(positiveContexts, function(c) {
        return url.lastIndexOf(c, 0) === 0;
    });
