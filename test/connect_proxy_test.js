...
exports.connect_proxy = {
    ...
    default_options: function(test) {
        ...
        test.equal(
            proxies[0].config.xforward,
            false,
            "should have default xforward"
        );
        test.equal(
            proxies[0].config.ws,
            false,
    ...
    full_options: function(test) {
        ...
        test.equal(
            proxies[1].config.context,
            "/full",
            "should have context set from config"
        );
        test.equal(
            proxies[1].config.host,
            "www.full.com",
            "should have host set from config"
        );
        test.equal(
            proxies[1].config.port,
            8080,
            "should have port set from config"
        );
        test.equal(
            proxies[1].config.https,
            true,
            "should have http set from config"
        );
        test.equal(
            proxies[1].config.xforward,
            true,
            "should have xforward set from config"
        );
        test.equal(
            proxies[1].config.ws,
            true,
            "should have ws set from config"
        );
        test.deepEqual(
            proxies[1].config.rewrite,
            { "^/full": "/anothercontext" },
            "should have rewrite set from config"
        );
        test.equal(
            proxies[1].config.rules.length,
            1,
            "rules array should have an item"
        );
        test.deepEqual(
            proxies[1].config.rules[0],
            { from: new RegExp("^/full"), to: "/anothercontext" },
            "rules object should be converted to regex"
        );
    ...
    two_rewrites: function(test) {
        ...
        test.deepEqual(
            config.rewrite,
            { "^/context": "/anothercontext", test: "testing" },
            "should have rewrite set from config"
        );
    ...
    invalid_configs: function(test) {
        ...
        test.notEqual(
            proxies[0].config.context,
            "/missinghost",
            "should not have context set from config with missing host"
        );
        test.notEqual(
            proxies[0].config.host,
            "www.missingcontext.com",
            "should not have host set from config with missing context"
        );
        test.notEqual(
            proxies[1].config.context,
            "/missinghost",
    ...
    invalid_rewrite: function(test) {
        ...
        test.equal(
            ...
            1,
            "rules array should have one valid item"
        );
        test.deepEqual(
            proxies[3].config.rules[0],
            { from: new RegExp("^/in"), to: "/thisis" },
            "rules object should be converted to regex"
        );
    ...
    function_rewrite: function(test) {
        ...
        test.equal(
            ...
            "/",
            "should execute function when replacing"
        );
        test.done();
    },
};
